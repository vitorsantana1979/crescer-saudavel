#!/usr/bin/env python3
"""
Script para extrair dados dos PDFs INTERGROWTH baseado no texto extraído
"""
...
def extract_data_from_text(text):
    """Extrai dados das linhas de texto do PDF"""
    data = []
    lines = text.split('\n')
    ...
    for line in lines:
        ...
        if match:
            ...
            # Converte semanas+dias para semanas decimais
            age_weeks = weeks + (days / 7.0)
...
def process_intergrowth_pdf_text(pdf_path, output_path):
    """Processa PDF INTERGROWTH extraindo dados do texto"""
...
def main():
    """Função principal"""
    if len(sys.argv) < 2:
        print("Uso: python pdf_text_to_json.py <diretório_dos_pdfs>")
        print("Exemplo: python pdf_text_to_json.py ../docs/")
        sys.exit(1)
    ...
    for pdf_name, json_name in pdf_mappings.items():
        ...
        if process_intergrowth_pdf_text(pdf_path, output_path):
            success_count += 1
