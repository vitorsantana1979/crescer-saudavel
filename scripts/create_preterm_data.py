#!/usr/bin/env python3
"""
Script para criar dados de prematuros com semanas+dias específicos
...
def create_preterm_data():
    """Cria dados de prematuros com semanas+dias específicos"""
    
    # Dados base para prematuros (meninos e meninas têm valores similares)
    preterm_base_data = [
        # Semana 24
...
def main():
    """Função principal"""
    print("🔄 Criando dados de prematuros com semanas+dias específicos")
    ...
    print(f"✅ Salvo: {boys_path.name} ({len(preterm_data)} pontos)")
    ...
    print(f"✅ Salvo: {girls_path.name} ({len(preterm_data)} pontos)")
    
    print(f"\n🎉 Dados de prematuros criados com sucesso!")
    print(f"📊 Total: {len(preterm_data)} pontos para cada sexo")
    print(f"📅 Faixa: 24+0 a 36+6 semanas")
